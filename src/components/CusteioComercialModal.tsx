...
export default function CusteioComercialModal({ tables, onClose }: Props) {
  const withActivity = tables.filter(t => t.novo > 0 || t.emAlteracao > 0)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 overflow-y-auto" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-2 flex items-center gap-4 text-xs text-on-surface-variant border-b border-outline-variant/40">
        ...
        <div className="px-6 py-4">
          {withActivity.length === 0 ? (
            <div className="text-sm text-outline text-center py-6">Nenhum componente em custeio no momento.</div>
          ) : (
              ...
              {withActivity.map(t => (
                <li key={t.tableName} className="flex items-center justify-between gap-3 py-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs font-mono mt-0.5 flex items-center gap-3">
