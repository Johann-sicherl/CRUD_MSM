...
export async function POST(request: NextRequest) {
  ...
  try {
    const { codes, description, foundInProtheus } = await fetchStructureCodes(protheusCode, { user, password })
    return NextResponse.json({ codes, description, foundInProtheus })
  } catch (err) {
