...
import { PDM_FIELD_MAP, comparePdmWithSupabase, displayText, type PdmComparisonRow } from '@/lib/pdmCompare'

// Compara o catálogo de Cadastro de Componentes (Supabase) com o que está
// validado no PDM (AC_VALIDADO = 'S'), pra visibilidade de divergências —
// tela exclusiva do perfil Administrador (ver Sidebar.tsx e o guard abaixo).
//
...
//  - vermelho esmaecido na LINHA: existe no PDM, não existe no Supabase —
//    precisa ser cadastrado em Cadastro de Componentes.
//  - cinza esmaecido na LINHA, sem interação: existe no Supabase, não veio
...
export default function PdmConsultaAcessoriosPage() {
  ...
  const fetchSupabaseSide = useCallback(async () => {
    ...
    try {
      const [pdmRes] = await Promise.all([
        fetch('/api/pdm-consulta-acessorios', {
          ...
          body: JSON.stringify({ user: pdmCreds.user, password: pdmCreds.password }),
        }),
        fetchSupabaseSide(),
      ])
  ...
  const counts = useMemo(() => {
    ...
    return {
      ...
      supabaseOnly: comparison.filter(r => r.status === 'supabase-only').length,
    }
  }, [comparison])
    ...
    <div className="p-8 max-w-[108rem]">
      <div className="mb-6">
        <div className="text-xs font-mono text-outline uppercase tracking-[0.2em] mb-1">
          Consulta Banco de Dados · consulta pdm x supabase
        </div>
        <h1 className="text-3xl font-bold text-on-surface tracking-tight">Consulta PDM x Supabase</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        >
          {loading ? 'Consultando...' : pdmCreds ? 'Consultar PDM' : 'Conectar e consultar PDM'}
        </button>
        {counts && (
          <div className="flex items-center gap-2 text-xs font-mono">
            ...
            <span className="px-2 py-1 rounded border border-outline-variant text-outline">{counts.supabaseOnly} só no Supabase</span>
          </div>
        )}
      </div>
      ...
      {comparison && (
        <>
          <div className="flex flex-wrap items-center gap-4 text-xs text-on-surface-variant mb-3">
            ...
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-surface-container-highest border border-outline-variant inline-block opacity-60" /> só existe no Supabase — falta inserir no PDM</span>
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      {PDM_FIELD_MAP.filter(f => f.supabaseField !== 'legacy_group_id').map(f => {
                        ...
                        return (
                          <td key={f.supabaseField} className={`px-4 py-2.5 text-on-surface-variant whitespace-nowrap ${isDiff ? 'bg-error/15' : ''}`}
                            title={isDiff ? `Supabase: ${row.status !== 'pdm-only' && row.status !== 'supabase-only' ? displayText(row.supabaseRow[f.supabaseField]) : ''}` : undefined}
                          >
                            {value}
                          </td>
                        )
                      ...
                      <td className="px-4 py-2.5 text-right whitespace-nowrap">
                        {isPdmOnly && <span className="text-error text-xs font-medium">Não cadastrado</span>}
      ...
      {editRecord && (
        <RecordModal
          ...
          onSaved={() => { setEditRecord(null); fetchSupabaseSide() }}
        />
      )}
    </div>
  )
